...
def test_sft_train(model_name, expected):
    ...
    '512', '--adam_epsilon', '1e-08', '--packing', 'False', '--num_bucket', '8', '--subset', "''", ]
    if '72' in model_name:
        command += ['--max_steps', '50', '--gradient_checkpointing', 'True', '--pipelining_fwd_bwd', 'True']  ## TODO create ds_config.json
    else:
    ...
    assert train_samples_per_second > 0.9 * train_samples_per_second_expected, f"Got {train_samples_per_second}, expected 0.9*{train_samples_per_second_expected}"
