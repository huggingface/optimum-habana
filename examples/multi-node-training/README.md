...
> Note that one should only set `HCCL_OVER_OFI=1` and `LD_LIBRARY_PATH=/root/hccl_ofi_wrapper:/opt/amazon/openmpi/lib:/opt/amazon/efa/lib` on AWS DL1 instances. *These should not be used otherwise*.
