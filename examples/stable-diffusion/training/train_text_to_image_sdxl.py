...
"""
Fine-tuning script for Stable Diffusion models for text2image with support for LoRA.
...
def main(args):
    ...
    # Select appropriate pipeline version
    sdxl_models = ["stable-diffusion-xl", "sdxl"]
    sdxl = True if any(model in args.pretrained_model_name_or_path for model in sdxl_models) else False
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt and args.num_validation_images > 0:
            ...
            with torch.autocast(device_type="hpu", dtype=weight_dtype, enabled=gaudi_config.use_torch_autocast):
                images = [
                    pipeline(args.validation_prompt, num_inference_steps=25).images[0]
