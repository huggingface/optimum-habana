...
###############################################################################
# This script is adapted from LoRA fine-tuning approach described in run_lora_clm.py
# https://github.com/huggingface/optimum-habana/blob/v1.19.1/examples/language-modeling/run_lora_clm.py
# for vision-language models, adding multimodal dataset handling, vision tower
# management, and support for LLaVA, Qwen2.5-VL, and Gemma-3 model families.
############################################################################
...
class MultimodalVQADataset(Dataset):
    ...
    def __init__(
        ...
        if raw_dataset is not None:
            ...
            def _load_with_fallback(primary_name: str, fallback_names: List[str]):
                ...
                raise (
                    last_error
                    if last_error
                    else RuntimeError(f"Failed to load dataset '{primary_name}' and fallbacks")
                )
        ...
        logger.info(f"Dataset schema: image={self.image_col}, question={self.question_col}, answer={self.answer_col}")
...
def get_lora_config(model_config: Dict[str, Any], lora_rank: int = 16) -> LoraConfig:
    ...
    if family == "llava":
        ...
        logger.info("LLaVA model detected - will retarget LoRA to language-model linear layers after model load")
    else:
...
def configure_llava_lora_targets(model, lora_config: LoraConfig) -> LoraConfig:
    ...
    if selected_modules:
        ...
        logger.warning("No language-model LoRA targets detected for LLaVA; falling back to default target modules")
...
class MultimodalVLMTrainer:
    ...
    def _setup_logging(self):
        ...
        log_file = os.path.join(self.args.output_dir, f"training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
    ...
    def load_model_and_processor(self):
        ...
        try:
            ...
            model_cls = (
                AutoModelForVision2Seq if self.model_config["model_class"] == "vision2seq" else AutoModelForCausalLM
            )
    ...
    def train(self):
        ...
        training_args = GaudiVLMTrainingArguments(
            ...
            eval_strategy="steps"
            if (getattr(self.args, "do_eval", False) and self.eval_dataset is not None)
            else "no",
    ...
    def _collate_fn(self, batch):
        ...
        # Process the batch
        try:
            enc = self.processor(text=full_texts, images=full_images, return_tensors="pt", padding=True)
        except Exception as e:
            logger.error(f"Failed to process batch: {e}")
            # Fallback: try processing without images
            enc = self.processor(text=full_texts, return_tensors="pt", padding=True)
...
def main():
    ...
    # Model
    parser.add_argument("--model_name_or_path", type=str, required=True, help="HF model ID or path")
    ...
    parser.add_argument("--max_eval_samples", type=int, default=None, help="Limit evaluation samples for debugging")
    ...
    # Hardware
    parser.add_argument("--use_habana", action="store_true", help="Use Habana Gaudi")
    parser.add_argument(
        "--gaudi_config_name",
        type=str,
        default=None,
    ...
    parser.add_argument("--use_lazy_mode", action="store_true", help="Use Habana lazy mode (dynamic shapes)")
    ...
    parser.add_argument("--deepspeed", type=str, default=None, help="DeepSpeed config file path for ZeRO optimization")

    # Output
    parser.add_argument("--output_dir", type=str, default="./output_vlm", help="Output directory")
