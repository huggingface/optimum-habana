...
class clip_coco_reader(MediaReaderNode):
    ...
    def __init__(self, name, guid, device, inputs, params, cparams, node_attr):
        ...
        for attr in ['root', 'slice_index', 'num_slices', 'drop_remainder', \
                    'pad_remainder', 'seed', 'shuffle', 'max_file', 'partial_batch']:
        ...
        imgids_from_compositekey = set([k[0] for k in self.keys])
        ...
        if (self.max_file == None):
            ...
            img_list = map(lambda x : os.path.join(self.root, x), self.images_slice.values())
    ...
    def __next__(self):
        ...
        batch = np.array([self.batch_size - (0, len(self.keys_slice_pad))[last_index > self.num_keys_slice]], dtype=dt.UINT32)
    ...
    def gen_output_info(self):
        ...
        shapes = [self.np_images_shape, self.np_ids_shape, self.np_inputids_shape, self.np_attention_shape, self.np_batch_shape]
    ...
    def set_params(self, params):
        ...
        self.np_attention_shape = np.array(
            [self.batch_size, 128], dtype=np.uint32)  # this is variable output
...
schema.add_operator("ClipCocoReader", None, 0, 0, [],
                    5, coco_reader_params, None, clip_coco_reader, dt.NDT)
...
class ClipHPUMediaPipe(MediaPipe):
    instance_count = 0
    def __init__(self, root, dataset, batch_size, shuffle, drop_last, prefetch_count, num_instances, instance_id, device, train_mode):
        ...
        pipename = "{}:{}".format(
            self.__class__.__name__, ClipHPUMediaPipe.instance_count)
        pipename = str(pipename)

        super().__init__(device=device, batch_size=batch_size,
                         prefetch_depth=prefetch_count, pipe_name=pipename)
    ...
    def definegraph(self):
        ...
        self.input = fn.ClipCocoReader(root=self.root, seed=seed_mediapipe, shuffle=self.shuffle,
                                    drop_remainder=self.drop_last, num_slices=self.num_instances, slice_index=self.instance_id, partial_batch=output_partial, dataset=self.dataset)
        jpegs, ids, inputids, attention, batch = self.input()
        self.decode = fn.ImageDecoder(
                output_format=it.RGB_P, resize=[self.dataset.decode_width, self.dataset.decode_height], resampling_mode=ft.BICUBIC, \
                decoder_stage=ds.ENABLE_ALL_STAGES, random_crop_type=rct.CENTER_CROP)
        ...
        self.cmn = fn.CropMirrorNorm(crop_w=self.dataset.decode_width, crop_h=self.dataset.decode_height, crop_d=0, dtype=dt.FLOAT32)
...
class ClipHabanaDataLoader():
    def __init__(self, *args, **kwargs):
        ...
        self.sampler = helper('sampler', 3, None) # TODO sasarkar: pass on sampler and use it for shuffling in mediapipe
        ...
        imgroot = list(set(['/'.join(imgpath.split('/')[:-1]) for imgpath in self.dataset['image_path']]))
        assert len(imgroot) == 1

        pipeline = ClipHPUMediaPipe(root=imgroot[0], batch_size=self.batch_size,
                                shuffle=True, drop_last=self.drop_last, prefetch_count=3,
                                num_instances=_get_world_size(), instance_id=_get_rank(), device="gaudi2", train_mode=True, dataset=self.dataset)
        self.iterator = HPUClipPytorchIterator(mediapipe=pipeline)
        self.__iter = None
        print(f"Running with Habana media DataLoader with num_instances = {pipeline.num_instances}, instance_id = {pipeline.instance_id}.")
    def __iter__(self):
