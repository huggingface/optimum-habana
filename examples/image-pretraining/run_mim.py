...
class MaskGenerator:
    ...
    def __call__(self):
        mask_idx = torch.randperm(self.token_count, device="hpu")[: self.mask_count]
        mask = torch.zeros(self.token_count, dtype=int, device="hpu")
        mask[mask_idx] = 1

        mask = mask.reshape((self.rand_size, self.rand_size))
        mask = mask.repeat(self.scale, self.scale)
