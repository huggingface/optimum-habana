#!/usr/bin/env python3
# PyTorch Training Script for TIMM models with Gaudi2 Graph Mode
# Copy from https://github.com/huggingface/pytorch-image-models/blob/main/train.py, enable it with gaudi2 graph mode
...
group.add_argument("--torchcompile-mode", type=str, default=None, help="torch.compile mode (default: None).")
...
parser.add_argument(
    "--device-modules", default=None, type=str, nargs="+", help="Python imports for device backend modules."
)
...
group.add_argument("--sched", type=str, default="cosine", metavar="SCHEDULER", help='LR scheduler (default: "cosine"')
group.add_argument(
    "--sched-on-updates",
    action="store_true",
    default=False,
...
group.add_argument(
    "--lr-k-decay", type=float, default=1.0, help="learning rate k-decay for cosine/poly (default: 1.0)"
)
...
group.add_argument(
    "--color-jitter-prob", type=float, default=None, metavar="PCT", help="Probability of applying any color jitter."
)
group.add_argument(
    "--grayscale-prob",
    type=float,
...
group.add_argument(
    "--gaussian-blur-prob", type=float, default=None, metavar="PCT", help="Probability of applying gaussian blur."
)
(
    group.add_argument(
        "--aa",
...
def main():
    ...
    if not args.lr:
        ...
        if not args.lr_base_scale:
            on = args.opt.lower()
            args.lr_base_scale = "sqrt" if any([on in ("ada", "lamb")]) else "linear"
