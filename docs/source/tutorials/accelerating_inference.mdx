...
HPU Graphs are integrated into the [`GaudiTrainer`](https://huggingface.co/docs/optimum/habana/package_reference/trainer) and the [`GaudiStableDiffusion Pipeline`](https://huggingface.co/docs/optimum/habana/package_reference/stable_diffusion_pipeline) so that one can use them very easily:
...
Models that run partially on CPU may not work with HPU Graphs.
In that case, inference should be run in lazy mode (`use_lazy_mode=True` and `use_hpu_graphs=False`).
