...
Check out the [documentation of the 🤗 Diffusers library](https://huggingface.co/docs/diffusers/api/pipelines/stable_diffusion#diffusers.StableDiffusionPipeline) for more information.


## How to generate images?

To generate images from text with Stable Diffusion on Gaudi, you need to instantiate two instances:
- A pipeline with [`GaudiStableDiffusionPipeline`](to-update). This pipeline supports *text-to-image generation*.
- A scheduler with [`GaudiDDIMScheduler`](to-update). This scheduler has been optimized for Gaudi.

When initializing the pipeline, you have to specify `use_habana=True` to deploy it on HPUs.
Furthermore, in order to get the fastest possible generations you should enable **lazy mode** or **HPU graphs** with `use_lazy_mode=True` or `use_hpu_graphs=True`.
...
Outputs can be PIL images or Numpy arrays. See [here](to-update) all the parameters you can set to tailor generations to your taste.
