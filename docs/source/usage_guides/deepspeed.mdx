...
- ZeRO-2, which additionally partitions the gradients across processes so that each process retains only the gradients corresponding to its portion of the optimizer states.
...
To use DeepSpeed on Gaudi, you need to install Optimum Habana and Habana's DeepSpeed fork with:
```bash
