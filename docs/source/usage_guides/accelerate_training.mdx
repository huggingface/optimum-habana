...
Besides, using `--dataloader_num_workers 1` should help in most cases as it enables to perform data loading in a thread different from the main one.
...
## Improving Training Performance with HPU Graphs
...
To cope with such host-bound workloads, you may want try enabling _HPU Graphs_ feature, which records the computational graph once, then only trigger it for execution much faster multiple times.
...
This option will wrap the model in `habana_frameworks.torch.hpu.ModuleCacher`, which automatically records _HPU Graph_ on the model's usage.
...
However, the potential performance gain could be dramatic, raging from +50/+100% for throughput.
