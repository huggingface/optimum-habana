...
This optimization is well-suited for models with a high cost of copying data from the host to the device. You can enable it with the argument `non_blocking_data_copy`.

**We recommend to use it on Gaudi2** where the host can continue to execute other tasks (e.g. graph building) to get a better pipelining between the host and the device. On first-generation Gaudi, the device executing time is longer so one should not expect to get any speedup.
