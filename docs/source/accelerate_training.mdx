...
They are all compatible with each other and can be coupled with [distributed training](https://huggingface.co/docs/optimum/main/en/habana_distributed).
...
In order to not take them into account in the computation of the throughput at the end of the training, you can add the following training argument: `throughput_warmup_steps=2`.
...
In order to apply HMP, you must set `"use_habana_mixed_precision"` to `true` and `"hmp_opt_level"` to `"O1"` in the Gaudi configuration file.
...
Habana probides a few custom operators that achieve better performance than their PyTorch counterparts on Gaudi.
...
You can pass the argument `gaudi_memory_stats=True` to display live memory statistics in the trainer's progress bar:
- `mem` refers to the *current* memory consumption,
- `max` refers to the *maximum* memory consumption reached during the run,
- `total` refers to the *total* memory on the device.
