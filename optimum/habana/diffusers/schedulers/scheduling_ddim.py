...
class GaudiDDIMScheduler(DDIMScheduler):
    ...
    def get_params(self):
        if not self.are_timestep_dependent_params_set:
            ...
            self.alpha_prod_t_list = torch.stack(list(self.alpha_prod_t_list))
            self.alpha_prod_t_prev_list = torch.stack(list(self.alpha_prod_t_prev_list))
            self.variance_list = torch.stack(list(self.variance_list))
