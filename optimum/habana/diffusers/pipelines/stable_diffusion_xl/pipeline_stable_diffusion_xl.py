...
class GaudiStableDiffusionXLPipeline(
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None and zero_out_negative_prompt:
            ...
            negative_prompt = num_prompts * [negative_prompt] if isinstance(negative_prompt, str) else negative_prompt
            negative_prompt_2 = (
                num_prompts * [negative_prompt_2] if isinstance(negative_prompt_2, str) else negative_prompt_2
            )
            ...
            if prompt is not None and type(prompt) is not type(negative_prompt):
                ...
                raise ValueError(
                    ...
                    f" {prompt} has batch size {num_prompts}. Please make sure that passed `negative_prompt` matches"
            ...
            for j in self.progress_bar(range(num_batches)):
                ...
                for i, t in enumerate(timesteps):
                    ...
                    if callback_on_step_end is not None:
                        ...
                        add_text_embeddings_batch = callback_outputs.pop("add_text_embeddings_batch", add_text_embeddings_batch)
    ...
    @torch.no_grad()
    def unet_hpu(
        self, latent_model_input, timestep, encoder_hidden_states, timestep_cond, cross_attention_kwargs, added_cond_kwargs, capture
    ):
        if self.use_hpu_graphs:
            return self.capture_replay(latent_model_input, timestep, encoder_hidden_states, timestep_cond, cross_attention_kwargs, added_cond_kwargs, capture)
        else:
            return self.unet(
                latent_model_input,
                timestep,
