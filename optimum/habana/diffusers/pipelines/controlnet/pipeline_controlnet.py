...
class GaudiStableDiffusionControlNetPipeline(
    ...
    def __call__(
        ...
        input_image: PipelineImageInput = None,
        ...
        **kwargs,
        ...
        Args:
            ...
            input_image (`torch.FloatTensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.FloatTensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
        ...
        with torch.autocast(device_type="hpu", dtype=torch.bfloat16, enabled=self.gaudi_config.use_torch_autocast):
            ...
            # 4. Prepare image
            if isinstance(controlnet, ControlNetModel):
              input_image = self.prepare_image(
                  image=input_image,
                  width=width,
                  height=height,
    ...
    @torch.no_grad()
    def unet_hpu(
        self, latent_model_input, timestep, encoder_hidden_states,  timestep_cond, cross_attention_kwargs, 
        down_block_additional_residuals, mid_block_additional_residual, capture
    ):
    ...
    def capture_replay(self, latent_model_input, timestep, encoder_hidden_states,
            down_block_additional_residuals, mid_block_additional_residual, capture):
        inputs = [latent_model_input, timestep, encoder_hidden_states, down_block_additional_residuals, mid_block_additional_residual, False]
        ...
        if capture:
            ...
            with self.ht.hpu.stream(self.hpu_stream):
                ...
                graph.capture_begin()
                outputs = self.unet(inputs[0], inputs[1], inputs[2], None, None, None, None, None, inputs[3], inputs[4], None, None, inputs[5])[0]
                graph.capture_end()
                graph_inputs = inputs
