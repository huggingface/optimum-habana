        ...
        if latents_batches[-1].shape[0] < batch_size:
            ...
            if negative_prompt_embeds_mask is not None:
                sequence_to_stack = (negative_prompt_embeds_mask_batches[-1],) + tuple(
                    torch.zeros_like(negative_prompt_embeds_mask_batches[-1][0][None, :]) for _ in range(num_dummy_samples)
                )
        ...
        **kwargs,
        ...
        for j in range(num_batches):
            ...
            if do_true_cfg:
                ...
                negative_prompt_embeds_mask_batches = torch.roll(negative_prompt_embeds_mask_batches, shifts=-1, dims=0)
