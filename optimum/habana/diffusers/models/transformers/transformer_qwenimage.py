...
class GaudiQwenEmbedRope(nn.Module):
    ...
    def _compute_video_freqs(self, frame, height, width, idx=0):
        ...
        if self.scale_rope:
            freqs_cos_height = torch.cat([freqs_neg_cos[1][-(height - height // 2) :], freqs_pos_cos[1][: height // 2]], dim=0)
            freqs_cos_height = freqs_cos_height.view(1, height, 1, -1).expand(frame, height, width, -1)
            freqs_sin_height = torch.cat([freqs_neg_sin[1][-(height - height // 2) :], freqs_pos_sin[1][: height // 2]], dim=0)
            freqs_sin_height = freqs_sin_height.view(1, height, 1, -1).expand(frame, height, width, -1)
            freqs_cos_width = torch.cat([freqs_neg_cos[2][-(width - width // 2) :], freqs_pos_cos[2][: width // 2]], dim=0)
            freqs_cos_width = freqs_cos_width.view(1, 1, width, -1).expand(frame, height, width, -1)
            freqs_sin_width = torch.cat([freqs_neg_sin[2][-(width - width // 2) :], freqs_pos_sin[2][: width // 2]], dim=0)
