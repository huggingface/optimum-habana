...
from .configuration_mllama import MllamaTextConfig
...
def gaudi_mllama_text_rmsnorm_forward(self, hidden_states):
    """
    Copied from MllamaTextRMSNorm.forward: https://github.com/huggingface/transformers/blob/main/src/transformers/models/mllama/modeling_mllama.py
...
class GaudiMllamaRotaryEmbedding(torch.nn.Module):
    def __init__(
        self,
        dim=None,
...
class GaudiMllamaLinearScalingRotaryEmbedding(GaudiMllamaRotaryEmbedding):
...
class GaudiLlamaDynamicNTKScalingRotaryEmbedding(GaudiMllamaRotaryEmbedding):
...
class GaudiMllamaDynamicNTKScalingRotaryEmbedding(GaudiMllamaRotaryEmbedding):
    def __init__(self, *args, **kwargs):
        logger.warning_once(
            "`LlamaDynamicNTKScalingRotaryEmbedding` is deprecated an will be removed in v4.45. Please use "
            "`LlamaRotaryEmbedding`, which now also does dynamic ntk scaling (simply pass the model config to "
...
class GaudiMllamaTextMLP(MllamaTextMLP):
    def __init__(self, config):
        super(MllamaTextMLP, self).__init__()
        self.config = config
...
class TPGaudiMllamaTextMLP(GaudiMllamaTextMLP, TPModule):
    def __init__(
        self,
        config,
        ...
        GaudiMllamaTextMLP.__init__(self, self.config)
    ...
    @staticmethod
    def import_module(glu: GaudiMllamaTextMLP, group: ProcessGroup) -> "TPGaudiMllamaTextMLP":
        config = copy.deepcopy(glu.config)
        config.hidden_grow_factor = glu.config.intermediate_size / glu.config.hidden_size
        tp_glu = TPGaudiMllamaTextMLP(config=config, group=group)
        return tp_glu

    def pre_mlp_forward(self, x):
        out_par = GaudiMllamaTextMLP.pre_mlp_forward(self, x)
...
class GaudiMllamaTextSelfAttention(MllamaTextSelfAttention):
    ...
    def pre_attn_forward(
        ...
        **kwargs,
        ...
        """
        Copied from MllamaTextSelfAttention.forward: https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py
        The only differences are:
...
class TPGaudiLlamaAttention(GaudiMllamaTextSelfAttention, TPModule):
    def __init__(
        self,
        ...
        GaudiMllamaTextSelfAttention.__init__(self, self.config, layer_idx)
    ...
    @staticmethod
    def import_module(mha: GaudiMllamaTextSelfAttention, layer_idx, group: ProcessGroup) -> "TPGaudiLlamaAttention":
    ...
    def pre_attn_forward(
        ...
        **kwargs,
        ...
        hidden_states, attn_weights, present_key_value = GaudiMllamaTextSelfAttention.pre_attn_forward(
            self,
            hidden_states,
            attention_mask,
            position_ids,
...
class GaudiMllamaSelfAttentionDecoderLayer(MllamaSelfAttentionDecoderLayer):
    def __init__(self, config: MllamaTextConfig, layer_idx: int):
        super(MllamaSelfAttentionDecoderLayer, self).__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = GaudiMllamaTextSelfAttention(config=config, layer_idx=layer_idx)

        self.mlp = GaudiMllamaTextMLP(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        Copied from MllamaSelfAttentionDecoderLayer.forward: https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py
        The only differences are:
        - add new args token_idx
        - add new args attn_softmax_bf16
        - add new args reuse_cache
...
class GaudiMllamaTextModel(MllamaTextModel):
    ...
    def __init__(self, config: MllamaTextConfig):
        ...
        """
        super(MllamaTextModel, self).__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        self.embed_tokens = torch.nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        layers = []
        for layer_idx in range(config.num_hidden_layers):
            layer = GaudiMllamaSelfAttentionDecoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        """
        Copied from MllamaTextModel.forward: https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py
        The only differences are:
        - add new args token_idx
        - add new args attn_softmax_bf16
        - add new args reuse_cache
...
class GaudiMllamaForCausalLM(MllamaForCausalLM):
    """
    Inherits from MllamaForCausalLM: https://github.com/huggingface/transformers/blob/main/src/transformers/models/llama/modeling_llama.py
    The only differences are:
    - add new args token_idx
