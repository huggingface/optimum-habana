...
class GaudiLlamaConfig(LlamaConfig):
    def __init__(
        self,
        vocab_size=32000,
        hidden_size=4096,
...
@no_op_decorator
class GaudiLlamaRMSNorm(LlamaRMSNorm):
    def __init__(self, hidden_size, eps=1e-6, use_fused_rms_norm=True):
        super().__init__(hidden_size, eps)
        self.use_fused_rms_norm = use_fused_rms_norm

    def forward(self, hidden_states):
        if hidden_states.device.type == "hpu" and self.use_fused_rms_norm:
...
class GaudiLlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config, layer_idx: int):
        ...
        self.input_layernorm = GaudiLlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps, use_fused_rms_norm=config.use_fused_rms_norm)
        self.post_attention_layernorm = GaudiLlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps, use_fused_rms_norm=config.use_fused_rms_norm)
...
class GaudiLlamaPreTrainedModel(LlamaPreTrainedModel):
    config: GaudiLlamaConfig
    _no_split_modules = ["GaudiLlamaDecoderLayer"]
    _can_record_outputs = {
        "hidden_states": GaudiLlamaDecoderLayer,
        "attentions": GaudiLlamaAttention,
    }


class GaudiLlamaModel(GaudiLlamaPreTrainedModel):
    """
    ...
    def __init__(self, config):
        ...
        self.norm = GaudiLlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps, use_fused_rms_norm=config.use_fused_rms_norm)
...
class GaudiLlamaForCausalLM(GaudiLlamaPreTrainedModel, GaudiGenerationMixin):
    """
