...
class CoreAttention(torch.nn.Module):
    ...
    def gaudi_flash_attn_v1(self, query_layer, key_layer, value_layer, attention_mask, softmax_mode):
        ...
        q_tiles = (q_len // self.q_block_size) if (q_len % self.q_block_size == 0) else math.ceil(q_len / self.q_block_size)
                ...
                if flash_attention_causal_mask:
                    ...
                    with ht.sdp_kernel(enable_recompute=flash_attention_recompute):
                        ...
                        if (q_len > 16384 or (q_len >= 6144 and bsz >= 2)) and self.training:
                            attn_output = self.gaudi_flash_attn_v1(query_layer, key_layer, value_layer, attention_mask, dropout_rate, softmax_mode)
                        else:
                            attn_output = self.fused_scaled_dot_product_attention(
                                query_layer, key_layer, value_layer, attention_mask, dropout_rate, False, None, softmax_mode
                            )
...
CORE_ATTENTION_CLASSES = {
    "eager": CoreAttention,
    "sdpa": CoreAttention,
    "flash_attention_2": CoreAttention
}
...
class SelfAttention(torch.nn.Module):
    ...
    def allocate_kv_cache(self, batch_size, max_seq_len, inp_seq_len):
        cache_shape = (batch_size, self.num_multi_query_groups_per_partition, max_seq_len, self.hidden_size_per_attention_head)
        ...
        if use_cache:
            ...
            if reuse_cache:
                ...
                if past_key_value is None:
                    past_key = torch.zeros(key_layer.shape, dtype=self.query_key_value.weight.dtype, device=key_layer.device)
...
class ChatGLMModel(ChatGLMPreTrainedModel):
    ...
    def _prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length, pre_seq_len):
        # create causal mask
        ...
        if attention_mask is not None:
            ...
            if pre_seq_len > 0:
                pre_seq_mask = torch.zeros([input_shape[0], 1, 1, pre_seq_len], dtype=expanded_attn_mask.dtype, device=expanded_attn_mask.device)
    ...
    def forward(
        ...
        if self.pre_seq_len > 0:
            ...
            if past_key_values is None:
                prefix_encoders = self.get_prompt(batch_size=batch_size, device=input_ids.device,
        ...
        **kwargs
        ...
        if past_key_values:
            if images is not None:
                assert hasattr(self.config, "vision_config")
                position_ids = position_ids[..., -1:] + token_idx - position_ids.size(-1) + self.num_patches
            else:
